% === T16 - Procesos del Sistema Operativo ===
...
\begin{frame}[fragile]{Estados de un proceso}
    ...
    \end{itemize}
    \uncover<6->{Pasar de \textbf{En Ejecución} a \textbf{Preparado} es posible cuando se \emph{pausa} o \emph{desalojo} un proceso.}
    \begin{itemize}
    \setlength\itemsep{0cm}
...
\begin{frame}{Colas de espera}
    ...
    \bigskip
    \uncover<3->{Si el proceso decide ser \emph{pausado}, el sistema denomina \textbf{cooperativo}.}\\
    \medskip
    \uncover<4->{Si el proceso es \emph{desalojado} en cualquier momento por el sistema, se dice que el sistema tiene \textbf{desalojo}.}\\
    \end{textblock}
\end{frame}
...
\begin{frame}[t]{Cambio de contexto (\emph{context switch})}
    ...
    Aun así, actualizar las estructuras de administración de memoria es muy costo.
    \end{tcolorbox}
    }
    \end{textblock}
\end{frame}
...
\begin{frame}[t]{\emph{Threads} (Hilos)}
    ...
    \bigskip
    \uncover<2->{Los \emph{threads} son \textbf{nuevos contexto de ejecución}, similares a un nuevo proceso, pero que comparten recursos con el proceso padre.
